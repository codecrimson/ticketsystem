...
const getUniqueErrorMessage = function (err) {
  ...
  try {
    ...
    if (err.errmsg.lastIndexOf('.$') !== -1) {
      ...
      // "errmsg" : "E11000 duplicate key error index: reerticket-dev.users.$email_1 dup key: { : \"test@user.com\" }"
      begin = err.errmsg.lastIndexOf('.$') + 2;
    } else {
      // support mongodb >= 3.2 (default: WiredTiger engine)
      // "errmsg" : "E11000 duplicate key error collection: reerticket-dev.users index: email_1 dup key: { : \"test@user.com\" }"
